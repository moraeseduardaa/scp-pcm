        ...
        <div class="col-md-5">
          <div class="card shadow-lg">
            <div class="card-body text-center p-5">
              ...
              <div class="mb-4">
                <input v-model="usuario" type="text" class="form-control form-control-lg mb-3" placeholder="Usuário"
                  style="background-color: #f8f9fa; color: #000; border: 2px solid #dc3545;"
                  @input="usuario = usuario.toUpperCase()">
                <input v-model="senha" type="password" class="form-control form-control-lg" placeholder="Senha"
                  style="background-color: #f8f9fa; color: #000; border: 2px solid #dc3545;">
              </div>
              <div v-if="erroLogin" class="mb-3 text-danger">{{ erroLogin }}</div>
              
              <div class="buttons">
                <button class="btn btn-secondary px-5 mt-3 me-2" @click="emit('voltar-login')"
                  style="border-radius: 50px; padding: 10px 12px; font-size: 17px;">
                  <i class="bi bi-arrow-left-circle me-2"></i>
                  Voltar
                </button>
                <button class="btn btn-danger px-5 mt-3" @click="fazerLogin" :disabled="loading"
                  style="border-radius: 50px; padding: 10px 12px; font-size: 17px;">
